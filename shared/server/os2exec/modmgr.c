...
extern void *_prog_img_start;

/* Some general ideas here:
     We use static array of modules structures here. We limit now to 1024 module handles.
     Such approach is much protected in comparison to allowing direct access to module structure.
...
getrec(char *mname, struct module_rec **p)
{
  ...
  while(prev)
  {
    if(stricmp(mname, prev->mod_name)==0)
    {
...
/* void print_module_table()
{
  ...
  while((el = (struct module_rec *)el->next))
  {
  ...
  *phmod=0;
  ...
  if (! exeflag && ! stricmp(mname, "EMXWRAP"))
  {
    if (options.debugmodmgr) 
        io_log("ModLoadModule: EXMWRAP module replaced by SUB32 module.\n");
    //mname="SUB32";
  ...
  if (options.debugmodmgr)
  {
      io_log("ModLoadModule: Loading module %s...\n", mname);
  }
...
unsigned long ModInitialize(void)
{
  ...
  module_root.mod_name = (char *)"root";
                               ...
                               void **       ppfn)
{
  ...
  { // Search by ordinal
    ...
    { // This is forward to another module. Call ourself...
      ...
      while(prev)
      {
        if(stricmp(ixfModule->Entries[ordinal-1].ModuleName, prev->mod_name)==0)
        {
          if(prev->load_status == LOADING)
          {
            searched_hmod=0;
       ...
       entries_counter++)
  {
    ...
    { // Found!
      ...
      { // This is forward to another module. Call ourself...
        ...
        while(prev)
        {
          if(stricmp(ixfModule->Entries[entries_counter-1].ModuleName, prev->mod_name)==0)
          {
            if(prev->load_status == LOADING)
            {
              searched_hmod=0;
...
void ModLinkModule (IXFModule *ixfModule, unsigned long *phmod)
{
       ...
       imports_counter++)
  {
    ...
    while(prev)
    {
      ...
      //if (!stricmp(name, "EMXWRAP"))
      //  strcpy(name, "SUB32");

      if (!stricmp(ixfModule->Fixups[imports_counter].ImportEntry.ModuleName, prev->mod_name))
      {
        if(prev->load_status == LOADING)
        {
          *phmod=0;
    ...
    *phmod=(unsigned long)prev->module_struct;
    ...
    relative_jmp = ((long)(ixfModule->Fixups[imports_counter].ImportEntry.Address) - (long)(ixfModule->Fixups[imports_counter].SrcAddress) - 4);
    absolute_jmp = (unsigned long)ixfModule->Fixups[imports_counter].ImportEntry.Address;
...
int find_case_file(char *file_to_find, char *path, char *buffer_of_found_file, int buf_len) {
    ...
    while(diren) {
        ...
        if(stricmp(diren->d_name, file_to_find)==0) {
