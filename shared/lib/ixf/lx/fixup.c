...
/* osFree OS/2 personal;ity internal */
#include <os3/modlx.h>
...
void print_struct_r32_rlc_info(struct r32_rlc *rlc)
{
    ...
    io_log("\n");
    io_log("   %02X    %02X   src off = %04X   ", rlc->nr_stype, rlc->nr_flags, get_srcoff_cnt1_rlc(rlc));

    if ((rlc->nr_flags & NRRTYP) == NRRINT)
        io_log("object #    = %02d   ",   get_mod_ord1_rlc(rlc), get_mod_ord1_rlc(rlc));
    else
        io_log("mod ord #   = %02d   ",   get_mod_ord1_rlc(rlc), get_mod_ord1_rlc(rlc));

    if ((rlc->nr_flags & NRRORD) == NRRORD)
        io_log("import ord #     = %02d\n", get_imp_ord1_rlc(rlc));
    if ((rlc->nr_flags & NRRNAM) == NRRNAM)
        io_log("import name      = %s\n", get_imp_name_rlc(rlc));

    if(((rlc->nr_flags & NRRTYP) == NRRINT) && (source_type_mask != NRSSEG))
        io_log("target off       = %04X\n", get_imp_ord1_rlc(rlc));
//        io_log("  trgoff1,2: %4.d (0x%.4x)  \n", get_imp_ord1_rlc(rlc), get_imp_ord1_rlc(rlc));

  /* if (source_type_mask == NRSTYP)
          io_log("    0x0F: Source type mask.\n"); */
#if 0
    if (source_type_mask == NRSBYT)
        io_log("    0x00: Byte fixup (8-bits).\n");
    if (source_type_mask == NRSSEG)
        io_log("    0x02: 16-bit segment (16-bits).\n");
    if (source_type_mask == NRSPTR)
        io_log("    0x03: 16:16 pointer (32-bits).\n");
    if (source_type_mask == NRSOFF)
        io_log("    0x05: 16-bit offset (16-bits).\n");
    if (source_type_mask == NRPTR48)
        io_log("    0x06: 16:32 pointer (48-bits).\n");
    if (source_type_mask == NROFF32)
        io_log("    0x07: 32-bit offset (32-bits).\n");
    if (source_type_mask == NRSOFF32)
        io_log("    0x08: 32-bit Self-relative offset fixup(32-bits).\n");
    if ((rlc->nr_stype & NRALIAS) == NRALIAS)
        io_log("    0x10: Fixup to Alias Flag.\n");
    if ((rlc->nr_stype & NRCHAIN) == NRCHAIN)
        io_log("    0x20: Source List Flag.\n");

    io_log("Flags: 0x%x \n", rlc->nr_flags);
    if ((rlc->nr_flags & NR8BITORD) == NR8BITORD)  // 0x80
        io_log("    0x80: 8-bit Import Ordinal Flag. \n");

    if (((rlc->nr_flags & NR32BITOFF) != NR32BITOFF)  // 0x10
        && ((rlc->nr_flags & NR8BITORD) != NR8BITORD))
        io_log("   !0x10: 16-bit Import Ordinal Flag. \n");

    if ((rlc->nr_flags & NRRORD) == NRRORD)
        io_log("    0x01: Import by Ordinal Flag. \n");

    if ((rlc->nr_flags /*& NRRINT*/) == NRRINT)
        io_log("    0x00: Internal reference. \n");

    if ((rlc->nr_flags & NRRNAM) == NRRNAM)
        io_log("    0x02: Import by name. \n");
    if ((rlc->nr_flags & NRADD) == NRADD)
        io_log("    0x04: Additive fixup. \n");
    if ((rlc->nr_flags & NRICHAIN) == NRICHAIN)
        io_log("    0x08: Internal Chaining Fixup. \n");
    if ((rlc->nr_flags & NRRENT) == NRRENT)
        io_log("    0x03: Internal entry table fixup. \n");
    if ((rlc->nr_flags & NR32BITOFF) == NR32BITOFF)
        io_log("    0x10: 32-bit Target Offset. \n");
    if ((rlc->nr_flags & NR32BITADD) == NR32BITADD)
        io_log("    0x20: 32-bit Additive fixup. \n");
    if ((rlc->nr_flags & NR16OBJMOD) == NR16OBJMOD)
        io_log("    0x40: 16-bit Object/Module ordinal. \n");
    else
        io_log("   !0x40:  8-bit Object/Module ordinal. \n");
#endif

    if ((rlc->nr_stype & NRCHAIN) == NRCHAIN)
    {   /* Print source list offsets. */
        int count1 = get_srcoff_cnt1_rlc(rlc);
        int i=0;

        io_log(" Source list offsets: ");

        for (i=0; i<count1; i++)
        {
            int src_off = get_srcoff12_rlc(rlc, i);
            io_log(" %d (0x%x)", src_off, src_off);
        }
    ...
    if ((rlc->nr_flags & NRADD) == NRADD)
        io_log(" Additive fixup: %d\n", get_additive_rlc(rlc));
    /* io_log("  r32_soff=0x%x, r32_objmod=0x%x, r32_target.intref.offset32=0x%x, r32_target.intref.offset16=0x%x \n ",
...
int get_reloc_size_rlc(struct r32_rlc *rlc)
{
    ...
    if ((rlc->nr_stype & NRCHAIN) == NRCHAIN)
    {   /* Calculate size of source list offsets. */
        int count1 = get_srcoff_cnt1_rlc(rlc);
    ...
    io_log(" Warning! Unrecognized fixup in fixuplx.c:get_reloc_size_rlc() \n");
    io_log(" Risk for infinite loop! \n Type of fixup:\n");
    io_log("SRC: 0x%x, FLAGS: 0x%x \n", rlc->nr_stype, rlc->nr_flags);
...
void fake_pagefault_code(struct LX_module * lx_mod, unsigned int offs, struct t_os2process * proc) {
    ...
    for (i = 1; i <= lx_mod->lx_head_e32_exe->e32_objcnt; i++)
    {
        ...
        for (j = 1; j <= kod_obj->o32_mapsize; j++)
        {
            ...
            io_log("---offset(in code) in %lu > offs:%d < %lu ---\n",
                ...
                && (offs < (kod_sida->o32_pagedataoffset + kod_sida->o32_pagesize)))
            {
                ...
                io_log("Hittar offs i sida %d i objekt %d logisk_sida %d\n", j, i, logisk_sida);
                break;
            }
        }
    ...
    if (!hittat_sida)
        io_log("Can't find offset for page in code/data object! offs:%d\n", offs);
    ...
    {   // 32-bit self-relative offset (32-bits)
        ...
        *iptr_patch_offs = (unsigned long int) iptr_rel_dos_put_ptr;
        io_log("Efter fixup: (unsigned int *) *patch_offs: 0x%x \n", *((unsigned int*)patch_offs));
    }
    else
    {
        io_log("Unknown fixup! source type:%d flags:%d \n", min_rlc->nr_stype, min_rlc->nr_flags );
    }
